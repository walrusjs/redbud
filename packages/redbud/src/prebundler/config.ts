 ...
 */
function getTypeFieldForPkg(pkg: Api['pkg']): string | undefined {
...
function getDtsConfig(opts: { cwd: string; pkgPath: string; dtsPath: string; outputPath: string }) {
  return {
...
export function getConfig(opts: {
  userConfig: RedbudPreBundleConfig;
  cwd: string;
  pkg: Api['pkg'];
}) {
  const pkgExternals: RedbudPreBundleConfig['extraExternals'] = Object.keys(
  ...
  Object.entries(deps).forEach(([dep, depConfig]) => {
    ...
    const depTypeInfo = depConfig.dts !== false ? getTypeInfoForPkg(depPkgPath) : null;
  ...
  extraDtsDeps.forEach((pkg) => {
    ...
    if (depTypeInfo) {
      config.dts[depTypeInfo.dtsPath] = getDtsConfig({
        ...
        dtsPath: depTypeInfo.dtsPath,
        outputPath: path.resolve(opts.cwd, `${DEFAULT_OUTPUT_DIR}/${pkg}/index.d.ts`)
      });
    }
  });
  ...
  Object.values(config.deps).forEach((depConfig) => {
    const rltDepExternals = Object.entries(depExternals).reduce<Record<string, string>>(
      (r, [dep, target]) => {
        // skip self
        if (dep !== depConfig.pkg.name) {
          // transform dep externals path to relative path
          r[dep] = winPath(path.relative(path.dirname(depConfig.output), path.dirname(target)));
        }
