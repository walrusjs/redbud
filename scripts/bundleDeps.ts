...
Object.keys(exported).forEach(function (key) {
    ...
    get: function get() {
      ...
      let { code, assets } = await ncc(entry, {
        ...
        customEmit(filePath: string, { id }: any) {
          if (
            (opts.file === './bundles/webpack/bundle' && filePath.endsWith('.runtime.js')) ||
      ...
      for (const fileToCopy of filesToCopy) {
        ...
        fs.writeFileSync(path.join(target, path.basename(fileToCopy)), content, 'utf-8');
      }
      ...
      if (
        ['chalk', 'pkg-up', 'execa', 'globby', 'os-locale', 'copy-webpack-plugin'].includes(
          opts.pkgName
        )
      ) {
      ...
      if (opts.pkgName === 'mini-css-extract-plugin') {
        ...
        fs.copyFileSync(
          path.join(nodeModulesPath, opts.pkgName, 'dist', 'loader-options.json'),
      ...
      if (opts.pkgName === 'vite') {
        ...
        Object.entries<string>(compiledConfig.externals)
          .filter(
            ([name, target]) =>
              target.startsWith('@umijs/') && compiledConfig.extraDtsExternals.includes(name)
          )
          .forEach(([name, target]) => {
            fs.writeFileSync(
      ...
      if (opts.file === './bundles/babel/bundle') {
        ...
        readWantedLockfile(PATHS.ROOT, {
          ...
          const [, unicodeParentPkg] = Object.entries(lockfile!.packages!).find(([name]) =>
            name.startsWith('/regexpu-core/')
          )!;

          if (unicodeParentPkg.dependencies![unicodePkgName] !== pkg.dependencies[unicodePkgName]) {
        ...
        if (opts.pkgName === 'webpack-5-chain') {
          ...
          fs.writeFileSync(
            ...
            fs
              ...
              .replace(`} from 'webpack';`, `} from '@umijs/bunder-webpack/compiled/webpack';`),
            'utf-8'
          );
        }
  ...
  if (opts.file && !opts.dtsOnly) {
    const packagesDir = path.join(opts.base, path.dirname(opts.file), 'packages');
