...
const getSWCAlias = (opts: {
  fileAbsPath: string;
  alias: RedbudBundlessConfig['alias'];
  cwd: string;
...
const getModuleType = (type: RedbudBundlessTypes) => {
  if (type === RedbudBundlessTypes.CJS) {
    return 'commonjs';
  }
 ...
 * swc transformer
 */
const swcTransformer: JSTransformer = async function (content) {
